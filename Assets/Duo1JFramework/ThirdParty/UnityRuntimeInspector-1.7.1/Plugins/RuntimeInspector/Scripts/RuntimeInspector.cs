...
#pragma warning restore 0649
        ...
        {
            get { return m_inspectedObject; }
        }

        public bool IsBound
        {
            get { return !m_inspectedObject.IsNull(); }
        }
        ...
        protected override void Awake()
        {
            ...
            if (selectorRoot == null)
            {
                ...
                {
                    selectorRoot = transform.parent;
                }
                else
                {
                    selectorRoot = transform;
                }
            }

            ColorPicker.Instance.transform.SetParent(selectorRoot);
            ColorPicker.Instance.transform.localScale = Vector3.one;
            ColorPicker.Instance.Close();
            ObjectReferencePicker.Instance.transform.SetParent(selectorRoot);
            ObjectReferencePicker.Instance.transform.localScale = Vector3.one;
            ObjectReferencePicker.Instance.Close();
        }

        private void InitializeSizeControl()
        {
            ...
            if (widthSlider != null)
            {
                ...
                float oriWidth = rectTF.rect.width;
                widthSlider.onValueChanged.AddListener((val) => { rectTF.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, oriWidth * val); });
            }
            ...
            if (heightSlider != null)
            {
                ...
                float oriHeight = rectTF.rect.height;
                heightSlider.onValueChanged.AddListener((val) => { rectTF.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, oriHeight * val); });
            }
        }
        ...
        {
            sizeBackground.color = Skin.BackgroundColor;
            background.color = Skin.BackgroundColor;
            scrollbar.color = Skin.ScrollbarColor;

            if (chgColorTxt != null)
            {
                foreach (Text text in chgColorTxt)
                {
